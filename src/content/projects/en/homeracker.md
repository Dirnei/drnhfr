---
title: 'HomeRacker'
summary: 'Website and configurator for HomeRacker, the modular 3D-printable rack system by Patrick Pötz.'
role: 'Website redesign and configurator'
period: '2026–present'
stack: ['Astro', 'SVG']
featured: true
order: 30
translationKey: 'homeracker'
links:
  repo: 'https://github.com/kellerlabs/homeracker'
  docs: 'https://homeracker.org/'
  demo: 'https://homeracker.org/configurator/'
---

HomeRacker is a rack system you print yourself. All the parts snap together
without tools into a rack of whatever size is needed. The system is by Patrick
Pötz, under MIT and CC BY-SA. The website and the configurator are mine.

In the configurator you click a rack together, see it straight away and get
the bill of materials with it. The layout sits in the address bar, so a
configuration can be passed on as a link.
